...
const Cart = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <Header />
      <main className="container py-4 md:py-8 min-h-[60vh]">
        ...
        {items.length === 0 ? (
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="md:col-span-2 space-y-3">
              {items.map((item) => (
                <div key={item.product.id} className="flex gap-4 bg-card rounded-xl p-3 shadow-soft">
                  <Link to={`/product/${item.product.id}`} className="w-20 h-24 rounded-lg overflow-hidden shrink-0">
                    <img src={item.product.image} alt={item.product.name} className="w-full h-full object-cover" />
                  </Link>
                  <div className="flex-1 min-w-0">
                    <Link to={`/product/${item.product.id}`}>
                      <h3 className="font-body font-medium text-sm text-foreground line-clamp-1 hover:text-primary">{item.product.name}</h3>
                    </Link>
                    ...
                    <div className="flex items-baseline gap-2 mt-1">
                      ...
                      <span className="text-[11px] text-muted-foreground font-body line-through">₹{item.product.originalPrice.toLocaleString()}</span>
                    </div>
                    <div className="flex items-center gap-3 mt-2">
                      <div className="flex items-center border border-border rounded-lg overflow-hidden">
                        <button onClick={() => updateQuantity(item.product.id, item.quantity - 1)} className="p-1.5 hover:bg-muted"><Minus size={14} /></button>
                        <span className="px-3 text-sm font-body font-medium">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.product.id, item.quantity + 1)} className="p-1.5 hover:bg-muted"><Plus size={14} /></button>
                      </div>
                      <button onClick={() => removeFromCart(item.product.id)} className="p-1.5 text-destructive hover:bg-destructive/10 rounded-lg">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </div>
            ...
            <div className="bg-card rounded-xl p-5 shadow-card h-fit sticky top-32">
              ...
              <button className="w-full bg-primary text-primary-foreground font-body font-semibold py-3 rounded-xl hover:brightness-110 transition-all shadow-elevated">
                Proceed to Checkout
              </button>
